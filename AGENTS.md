...
AI agent runtime: a Slack bot that spawns Codex sessions per message, with thread-based session continuity and a persistent memory pipeline.
...
pnpm cli auth test     # Verify Slack connection + Codex CLI
...
pnpm cli auth test                          # Test Slack + Codex
...
│   ├── ClaudeRunner.ts         # Spawns `Codex` CLI, parses JSON output
...
3. Spawns `Codex -p "message" --resume <session_id> --output-format json`
4. Codex runs in the agent workspace (`GOLDFISH_WORKSPACE`) with full tool access
...
The agent's identity comes from the workspace's `AGENTS.md`, not from Goldfish itself. Goldfish is the plumbing; the workspace defines who the agent is.

## Key Design Decisions

**Codex CLI over API:** Runs on a Codex Max subscription at zero marginal cost. No API billing, no token counting, no rate limits. The `--resume` flag gives us session continuity for free.

**`--dangerously-skip-permissions`:** The bot runs unattended — it can't prompt a human for tool approvals. This flag is required. The agent workspace's `AGENTS.md` and hooks are the safety layer.
...
**SQLite (Kysely) for sessions:** Lightweight, zero-config, runs anywhere. Sessions table maps Slack threads to Codex session IDs. Messages table logs all exchanges.
...
**Slack mrkdwn formatting:** Codex outputs standard Markdown. `slackFormatter.ts` converts it (bold, headers, links, tables→lists). Messages over 3900 chars are split at paragraph boundaries.
